...
@Component({
  selector: "app-coa-level2",
  templateUrl: "./coa-level2.component.html",
})
export class CoaLevel2Component implements OnInit {
  @ViewChild("fileInput") fileInput: ElementRef;
  materialType: LevelTwo = new LevelTwo();
  accountType: { id: number; name: string }[] = [];
  coaLevelOne: { id: any; name: string }[] = [];
  levelTwoSr: string;
  urlHelper = UrlHelper;
  tableData: any;
  count: number;
  ...
  show(id?: number) {
    if (id) {
      ...
      this.editMode = false;
      this.displayModal = true;
      this.materialType = new LevelTwo();
      // this.fetchAccountType();
      // this.fetchLevelOne();
    }
  }
  ...
  // fetchLevelOne() {
  //
  //   this._materialTypeService.getAllSuggestion("COALevel01").subscribe(
  //     (response: any) => {
  //       this.coaLevelOne = response.items.map((employee: any) => ({
  //         id: employee?.id,
  //         name: employee?.name,
  ...
  getLvl3SerialNumber(employee: any) {
    ...
    this._materialTypeService
      .getSerialNumber(employee.id, "COALevel02")
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.messageService.add({
  ...
  onLevelOneChange(selectedId: string) {
    const selectedEmployee = this.coaLevelOne.find(
  ...
  headerMap: { [key: string]: string } = {
    ...
    "COA Level 1 Name": "coaLevel01Name",
  ...
  downloadTemplate() {
    ...
    // Extract headers from the headerMap (display names)
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      saveAs(blob, "COA_Level2_Upload_Template.xlsx");
    });
  }
  ...
  handleParsedData(data: any[]) {
    ...
    // Map display headers to backend keys
    const mapped = data.map((row: any) => {
      const mappedRow: any = {};
      for (const [header, key] of Object.entries(this.headerMap)) {
        mappedRow[key] = row[header]; // example: row["Name"] → mappedRow["name"]
      }
      return mappedRow;
    });

    // Validate required fields (based on backend field names)
