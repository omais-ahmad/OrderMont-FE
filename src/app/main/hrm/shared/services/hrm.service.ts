...
export class HrmService {
  ...
  // savePolicy(dto: any, target: string) {
  //
  //   console.log(dto);
  //   this.url = this.baseUrl;
  //   this.url += target + "/EmployeePolicyAllocation";
  //   return this.http.post(this.url, dto);
  // }
  savePolicy(dto: any, target: string) {
    const params = new HttpParams()
      .set("employeeId", dto.employeeId)
      .set("policyId", dto.policyId)
      .set("UserId", dto.UserId); // use correct casing if needed
  ...
  attendanceReport(target: string, startDate: string, endDate: string) {
    ...
    this.url = `${
      this.baseUrl
    }${target}/GenerateAttendanceSummary?${params.toString()}`;
  ...
  getForEdit(id: number, target: string) {
    this.url = this.baseUrl;
  ...
  getAllSuppliersAndClients() {
    this.url = `${this.baseUrl}Suggestion/suppliers-and-clients`;
    return this.http.get(this.url).pipe(
      map((response: any) => {
        console.log(response);
        return response["result"];
  ...
  //   this.url = this.baseUrl;
  //   this.url += "PurchaseInvoice/UploadDocuments";
  ...
  UploadImage(payload: { base64Images: string[] }) {
    this.url = this.baseUrl;
    this.url += "PurchaseInvoice/UploadDocuments";
