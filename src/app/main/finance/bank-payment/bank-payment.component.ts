...
export class BankPaymentComponent {
  constructor(
    ...
    this.bankPaymentForm = this.fb.group({
      bankCOALevel04Id: ["", [Validators.required]],
      referenceNumber: [""],
      GeneralLedgerId: [""],
      voucherNumber: [""],
      invoiceNumber: [""],
      ...
      referenceDate: [""],
      // linkedDocument: ["", [Validators.required]]
      linkedDocument: ["1"],
  ...
  fetchDropdownData(target: string) {
    this._hrmService.getAllSuggestion(target).subscribe((response: any) => {
      switch (target) {
        case "Bank":
          this.allBanks = response.items.map((x: any) => ({
            id: x.id,
            name: x.name,
          }));
          break;
        ...
        case "Tax":
          this.tax = response.items.map((x: any) => ({
            id: x.id,
            name: x.name,
          }));
          break;
  ...
  getVoucherNumber() {
    this._financeService
      .getVoucherNumber("BP", this.bankPaymentForm.value.issueDate, this.target)
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.msgService.add({
        ...
        .subscribe({
          next: (response) => {
            ...
            this.bankPaymentForm.patchValue({
              ...
              invoiceNumber: this.dataForEdit.invoiceNumber,
  ...
  save() {
    ...
    console.log("BP Data", this.bankPaymentForm.value);
    ...
    const finalMaturityDate = formValue.maturityDate
      ? moment(formValue.maturityDate).format("YYYY-MM-DD")
      : moment(fallbackDate).format("YYYY-MM-DD");

    const finalReferenceDate = formValue.referenceDate
      ? moment(formValue.referenceDate).format("YYYY-MM-DD")
      : moment(fallbackDate).format("YYYY-MM-DD");

    const finalIsCheque =
      typeof formValue.isCheque === "boolean" ? formValue.isCheque : false;

    const finalIsCrossedCheque =
      typeof formValue.isCrossedCheque === "boolean"
        ? formValue.isCrossedCheque
        : false;
    this.bankPaymentForm.patchValue({
      ...
      isCheque: finalIsCheque,
      isCrossedCheque: finalIsCrossedCheque,
    });
  ...
  update() {
    ...
    // ▸ Collect updated grid data
    const updatedDetails: any[] = [];
    this.gridApi.forEachNode((node) => updatedDetails.push(node.data));

    const fv = this.bankPaymentForm.value;
    const fallback = fv.issueDate || new Date();

    // ▸ Dates
    const finalReferenceDate = fv.referenceDate
      ? moment(fv.referenceDate).format("YYYY-MM-DD")
      : moment(fallback).format("YYYY-MM-DD");

    const finalMaturityDate = fv.maturityDate
      ? moment(fv.maturityDate).format("YYYY-MM-DD")
      : moment(fallback).format("YYYY-MM-DD");

    // ▸ Booleans
    const finalIsCheque = !!fv.isCheque;
    const finalIsCrossedCheque = !!fv.isCrossedCheque;

    // ▸ Patch values before update
    this.bankPaymentForm.patchValue({
      generalPaymentDetails: updatedDetails,
      issueDate: moment(fallback).format("YYYY-MM-DD"),
      referenceDate: finalReferenceDate,
      maturityDate: finalMaturityDate,
      isCheque: finalIsCheque,
      isCrossedCheque: finalIsCrossedCheque,
    ...
    this._financeService
      .update(updatePayload, this.target)
  ...
  onCellValueChanged(event: any) {
    ...
    // Pick editable values
    const base = Number(data.netAmount) || 0; // main amount column
    const tax = Number(data.taxAmount) || 0;
    const otherTax = Number(data.otherTaxAmount) || 0;

    // If base/net amount is entered
    if (base > 0) {
      // Recalculate the final amount (could be called gross, but here we keep in netAmount)
      data.netAmount = base + tax + otherTax;

      this.calculateTotalAmount();

      // Refresh this row's "Amount" column
  ...
  calculateTotalAmount() {
    ...
    if (this.gridApi) {
      this.gridApi.forEachNode((node) => {
        if (node.data && node.data.netAmount) {
  ...
  selectPayment(adjustid: number) {
    ...
    // Exclude id if you don’t want to send it
    const { id, ...paymentWithoutId } = selectedPayment;

    // Patch form with proper array shape
    this.bankPaymentForm.patchValue({
