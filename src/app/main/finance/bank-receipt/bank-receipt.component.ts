...
@Component({
  ...
  styleUrl: "./bank-receipt.component.css",
})
export class BankReceiptComponent {
  ...
  getVoucherNumber() {
    this._financeService
      .getVoucherNumber("BR", this.bankPaymentForm.value.issueDate, this.target)
      .pipe(
        finalize(() => {}),
        catchError((error) => {
          this.msgService.add({
  ...
  save() {
    ...
    const fallbackDate = fv.issueDate || new Date();

    this.bankPaymentForm.patchValue({
      generalPaymentDetails: this.tempRowData,
      issueDate: moment(fv.issueDate).format("YYYY-MM-DD"),
      referenceDate: fv.referenceDate
        ? moment(fv.referenceDate).format("YYYY-MM-DD")
        : moment(fallbackDate).format("YYYY-MM-DD"),
      maturityDate: fv.maturityDate
        ? moment(fv.maturityDate).format("YYYY-MM-DD")
        : moment(fallbackDate).format("YYYY-MM-DD"),
      isCheque: !!fv.isCheque,
      isCrossedCheque: !!fv.isCrossedCheque,
    });
